...
async def create_book_with_chunks(
    ...
    await db.upsert_book(book_id, safe_title)
    ...
    logger.info("Created book %s (%s chunks, merged by word count)", book_id, len(rows))
...
class BookService:
    ...
    async def list_books(self) -> list[dict[str, Any]]:
        return await self._db.list_books()
    ...
    async def create_from_paragraphs(
        ...
        return await create_book_with_chunks(self._db, title, paragraphs, preferred_id)

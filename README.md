...
- `services/book_parser_service.py` — FB2 / EPUB → абзацы с **жирным/курсивом/ссылками** (Telegram HTML)
- `services/rich_text.py` — `TextSpan` и разметка для `parse_mode=HTML`
...
- `services/word_chunks.py` — цель **от 670 до 750** символов на сообщение; короткие абзацы (до ~200 символов) склеиваются со следующими; между абзацами в тексте — пустая строка
...
- `books` / `chunks` — текст; у книги есть **`owner_telegram_id`** (чужие книги в списке не попадают).

`current_position` = число **прочитанных чанков**; сообщение обычно **670–750** символов; абзацы короче **200** символов по возможности дополняются следующим (разделитель — пустая строка). Длинный абзац делится на несколько chunk'ов, границы по `\n` и пробелам, отступы сохраняются. Короткие `<p>` не выкидываются; EPUB без `<p>` — fallback на весь текст HTML; куски без текста пропускаются, парсинг идёт дальше.
